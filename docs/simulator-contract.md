...
| State-smart `ABORT"` | `ABORT"` remains immediate and has the same source-visible behavior on the emulator and simulator. Interpretation parses the quoted message and consumes `flag`: zero is a silent no-op, while nonzero prints the message and executes `ABORT`. Compilation retains the inline message and emits that conditional behavior for a runtime flag. This is a deliberate project extension where portable interpretation semantics are undefined. The exact Desktop closure currently contains seven top-level assertions in this form; that count explains the compatibility requirement but does not itself establish Desktop acceptance. |
...
checked `ENTROPY-FILL`/`ENTROPY-READY?`. Twenty-two following source words provide
