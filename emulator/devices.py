...
We carve out the first slice for our emulated peripherals:
  ...
  MMIO_BASE + 0x0C00 .. 0x0C1F  : One-shot PCM audio output

All registers are 8-bit accessed; the CPU's mem_read8/mem_write8 hits
these through the MMIO dispatch layer in emulator/system.py.
"""
...
class AudioOutput(AudioOutputModel, Device):
    """Map the shared PCM controller into the emulator's device bus."""

    def __init__(self, *, max_capture_bytes: int = AUDIO_MAX_CAPTURE_BYTES):
        Device.__init__(self, "Audio", AUDIO_BASE, AUDIO_SIZE)
        AudioOutputModel.__init__(self, max_capture_bytes=max_capture_bytes)
...
# ---------------------------------------------------------------------------
#  Executable ML-KEM-512 (Kyber) Key Encapsulation Device
# ---------------------------------------------------------------------------
# Implements deterministic ML-KEM-512 values for valid/generated keys.
...
class KemDevice(Device):
    """Synchronous executable ML-KEM-512 retained-buffer device.
    ...
    def _do_keygen(self):
        ek, dk = mlkem512_keygen(bytes(self._bufs[0]))
    ...
    def _do_encaps(self):
        ...
        c, K = mlkem512_encapsulate(ek, coin)
