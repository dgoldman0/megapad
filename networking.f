...
128 CONSTANT /TLS-SERVER-ACCEPT-OP
...
VARIABLE _TSAOM-GEN VARIABLE _TSAOM-LOWER
...
: _TSAO-BEGIN-PUBLISH
    ...
    _TSAOM-OP @ TSAO.STATE @ TLS-SERVER-ACCEPT-ST-BEGINNING = AND IF
        ...
        ELSE
            ...
            _TSAOM-OP @ TSAO.STATE !
        THEN
        _TSAOM-IOR @ _TSAOM-OP @ TSAO.RESULT-IOR !
    THEN
...
: _TSAO-STEP-CLAIM  ( op -- op epoch action ior )
    ...
    _TSAOM-OP @ _TSAO-CURRENT? IF
        ...
        _TSAOM-OP @ TSAO.STATE @ TLS-SERVER-ACCEPT-ST-WAIT-CHILD = IF
            _TSAOM-OP @ TSAO.CANCEL @ IF
                TLS-SERVER-ACCEPT-ST-ABORT-READY
                _TSAOM-OP @ TSAO.STATE !
                TLS-E-HANDSHAKE-CANCELLED _TSAOM-IOR !
            ELSE
                ...
                _TSAOM-OP @ TSAO.STATE !
                _TSAO-ACTION-ATTACH _TSAOM-ACTION ! TLS-E-OK _TSAOM-IOR !
            THEN
        ELSE
            _TSAOM-OP @ TSAO.STATE @ DUP
            TLS-SERVER-ACCEPT-ST-BEGINNING =
            ...
            ELSE
                _TSAOM-OP @ TSAO.STATE @ DUP
                TLS-SERVER-ACCEPT-ST-CLIENT-HELLO = IF
                    DROP TLS-E-BUSY _TSAOM-IOR !
                ELSE
                    TLS-SERVER-ACCEPT-ST-ABORT-READY =
                    _TSAOM-OP @ TSAO.STATE @
                    TLS-SERVER-ACCEPT-ST-LEASE-READY = OR IF
                        TLS-E-HANDSHAKE-CANCELLED _TSAOM-IOR !
                    THEN
                THEN
            THEN
...
: TLS-SERVER-ACCEPT-STEP  ( op -- sd progress alert ior )
    ...
    THEN DROP
    _TSAO-STEP-CLAIM
    >R DUP _TSAO-ACTION-ATTACH = IF
        DROP >R _TSAO-ATTACH-GUARDED R>
        _TSAO-ATTACH-PUBLISH R> DROP
    ELSE
        DROP 2DROP R>
        DUP TLS-E-HANDSHAKE-CANCELLED = IF
            >R 0 TLS-SERVER-ACCEPT-CLOSING 0 R>
        ELSE DUP TLS-E-BUSY = IF
            >R 0 TLS-SERVER-ACCEPT-BUSY 0 R>
        ELSE
            >R 0 TLS-SERVER-ACCEPT-TERMINAL 0 R>
        THEN THEN
    THEN ;
