    ...
    _TSAOM-OP @ _TSAO-CURRENT? IF
        ...
        _TSAOM-STATE @ TLS-SERVER-ACCEPT-ST-WAIT-CHILD = IF
            _TSAOM-OP @ TSAO.CANCEL @ IF
                ...
                TLS-E-HANDSHAKE-CANCELLED _TSAOM-IOR !
                0 TLS-E-HANDSHAKE-CANCELLED _TSAOM-OP @
                _TSAO-RESULT-LATCH
            ELSE
        ...
        ELSE _TSAOM-STATE @ TLS-SERVER-ACCEPT-ST-CLIENT-HELLO = IF
            ...
            ELSE _TSAOM-OP @ _TSAO-DEADLINE-EXPIRED? IF
                ...
                _TSAOM-OP @ TSAO.STATE !
                TLS-SERVER-ACCEPT-CLOSING _TSAOM-PROGRESS !
                TLS-E-HANDSHAKE-TIMEOUT _TSAOM-IOR !
                0 TLS-E-HANDSHAKE-TIMEOUT _TSAOM-OP @
                _TSAO-RESULT-LATCH
            ELSE
...
: _TSAO-CH-EARLY-DISPOSITION  ( alert -- )
...
: _TSAO-CH-RETRY  ( progress ior -- )
    _TSAOM-IOR ! _TSAOM-PROGRESS !
    0 _TSAOM-ALERT !
    TLS-SERVER-ACCEPT-ST-CLIENT-HELLO _TSAOM-OP @ TSAO.STATE ! ;

: _TSAO-CH-PROGRESSED  ( next-state -- )
    _TSAOM-OP @ TSAO.STATE !
...
: _TSAO-CH-MAP  ( -- )
    _TSAOM-OP @ TSAO.CANCEL @ IF
        TLS-E-HANDSHAKE-CANCELLED _TSAO-CH-ABORT-READY EXIT
    THEN
    _TSAOM-LOWER-IOR @ DUP TLS-E-WOULD-BLOCK = IF
        ...
        _TSAOM-LOWER-ALERT @ 0= AND IF
            ...
            _TSAO-CH-RETRY
        ELSE
            TLS-E-TRANSPORT _TSAO-CH-ABORT-READY
        THEN
        EXIT
    THEN
    DUP TLS-E-BUSY = IF
        ...
        _TSAOM-LOWER-PROGRESS @ TLS-SERVER-CLIENT-HELLO-NONE =
        _TSAOM-LOWER-ALERT @ 0= AND IF
            TLS-SERVER-ACCEPT-BUSY TLS-E-BUSY _TSAO-CH-RETRY
        ELSE
            TLS-E-TRANSPORT _TSAO-CH-ABORT-READY
        THEN
        EXIT
    ...
    DUP IF
        ...
        _TSAOM-LOWER-ALERT @ 0= AND R@ 0< AND IF
            R> _TSAO-CH-ABORT-READY
        ELSE
            R> DROP TLS-E-TRANSPORT _TSAO-CH-ABORT-READY
        THEN
        EXIT
    THEN DROP
    _TSAOM-LOWER-ALERT @ DUP IF
        ...
        _TSAOM-LOWER-PROGRESS @ TLS-SERVER-CLIENT-HELLO-NONE = AND IF
            _TSAOM-LOWER-ALERT @ _TSAO-CH-EARLY-DISPOSITION
        ELSE
            TLS-E-TRANSPORT _TSAO-CH-ABORT-READY
        THEN
        EXIT
