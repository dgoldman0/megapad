...
: TLS-ALPN-ACCEPT-SELECTION ( ctx ext-a ext-u -- ior )
    TLS-OWNER-TRY IF 2DROP DROP TLS-E-BUSY EXIT THEN
    2 PICK _TLS-SERVER-FLIGHT-ACTIVE? IF
...
: TLS-ENCRYPT-RECORD ( ctx ctype pt plen rec -- reclen )
    ...
    4 PICK _TLS-SERVER-FLIGHT-ACTIVE? IF
...
: TLS-DECRYPT-RECORD ( ctx rec rlen plain -- ctype plen | -1 0 )
    ...
    3 PICK _TLS-SERVER-FLIGHT-ACTIVE? IF
...
: TLS-SERVER-WORKSPACE-WIPE  ( ctx -- )
    DUP _TLS-SERVER-FLIGHT-ACTIVE? IF DROP EXIT THEN
...
: TLS-KS-HANDSHAKE ( ctx -- status )
    TLS-OWNER-TRY IF DROP TLS-E-BUSY EXIT THEN
    DUP _TLS-SERVER-FLIGHT-ACTIVE? IF
...
: _TSFT-SCRATCH-WIPE  ( -- )
    ...
    0 _TSFT-FIN-A ! 0 _TSFT-FIN-U ! 0 _TSFT-SIG-U !
...
: _TSFT-VALIDATE  ( -- ior )
    ...
    DUP TSM.FLAGS + @ TSMF-CLIENT-HELLO-VALID AND 0= IF
        DROP TLS-E-STATE EXIT
    THEN
    DUP TSM.FLIGHT-PHASE + @ TLS-SERVER-FLIGHT-SERVER-HELLO <>
    OVER TSM.PHASE-OFF + @ 0<> OR
    OVER TSM.TRANSCRIPT-PHASE + @ TLS-SERVER-TRANSCRIPT-CH-SH <> OR IF
        DROP TLS-E-STATE EXIT
    THEN DROP
...
: _TLS-TRANSPORT-FAILED? ( ctx -- flag )
    DUP _TLS-SERVER-FLIGHT-ACTIVE? IF DROP 0 EXIT THEN
...
: (TLS-IO-STATUS) ( ctx -- ior )
    ...
    DUP _TLS-SERVER-FLIGHT-ACTIVE? IF DROP TLS-E-BUSY EXIT THEN
...
: (TLS-SEND-DATA) ( ctx addr len -- actual )
    ...
    _TSD-CTX @ _TLS-SERVER-FLIGHT-ACTIVE? IF 0 EXIT THEN
...
: (TLS-RECV-DATA) ( ctx addr maxlen -- actual | -1 )
    ...
    _TRD-CTX @ _TLS-SERVER-FLIGHT-ACTIVE? IF 0 EXIT THEN
