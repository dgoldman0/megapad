...
\ One scratch-free predicate protects every upper-driver-owned server phase.
...
\ become ready for the same upper driver.
...
\ so an edge-triggered upper driver knows when to step again immediately.
...
\   record returns RECORD so an edge-triggered upper driver can step again.
...
\  Each socket is 344 bytes.  The common authority header is followed by one
...
\    +80  ALPN-LEN     8  copied ProtocolName length (0..255)
\    +88  ALPN       256  copied ProtocolName plus one zero pad byte
\
\  API:
...
\                 + 2×/SOCK = 238328 bytes per connection.
...
: SOCK.TLS-ALPN-LEN     ( sd -- addr ) 80 + ;
: SOCK.TLS-ALPN         ( sd -- addr ) 88 + ;
...
\ transaction in the global TLS -> credential -> NET order.  A child already
\ claimed by an upper driver owns its own exact TCB and credential pin; a
\ pre-claim close race is rejected by exact listener validation.
VARIABLE _STLL-SD
...
: _STLL-EXACT?  ( -- flag )
    ...
    SWAP SOCK.TLS-CRED-GEN @ _STLL-CRED-GEN @ = AND ;
