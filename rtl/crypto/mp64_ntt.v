...
module mp64_ntt (
    ...
    // mod_reduce — reduce a 64-bit value mod q (32-bit) via repeated
    // conditional subtraction at decreasing shifts.  Fully synthesisable.
    // For NTT with Kyber q=3329 (12-bit), the product is at most 64 bits.
    function [31:0] mod_reduce;
        input [63:0] val;
        input [31:0] q;
        reg [63:0] r;
        reg [63:0] qs;  // q shifted
        integer sh;
        begin
            r = val;
            // Start from the highest useful shift (product < 2^64, q >= 2)
            // and subtract q<<sh while r >= q<<sh, decreasing sh to 0.
            for (sh = 32; sh >= 0; sh = sh - 1) begin
                qs = {32'd0, q} << sh;
                // Subtract qs as many times as needed at this shift level.
                // Since q >= 2 and sh decreases, at most ~32 total subtracts.
                if (r >= qs) r = r - qs;
                if (r >= qs) r = r - qs;
                if (r >= qs) r = r - qs;
                if (r >= qs) r = r - qs;
            end
            // Final cleanup — ensure r < q
            if (r >= {32'd0, q}) r = r - {32'd0, q};
            mod_reduce = r[31:0];
        end
    endfunction
        ...
        if (!rst_n) begin
            ...
            if (req && wen) begin
                case (addr[5:3])
                    ...
                    3'b011: begin                             // LOAD_A
                        poly_a[idx_reg] <= mod_reduce({32'd0, wdata[31:0]}, q_reg);
                        idx_reg <= idx_reg + 8'd1;
                    end
                    3'b100: begin                             // LOAD_B
                        poly_b[idx_reg] <= mod_reduce({32'd0, wdata[31:0]}, q_reg);
                        idx_reg <= idx_reg + 8'd1;
                    end
                    default: ;
                endcase
