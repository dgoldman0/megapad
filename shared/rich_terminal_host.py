"""Backend-neutral state machine for an optional rich-terminal host port.

The state machine owns attachment epochs, bounded queues, and admission
ordering.  Backend hooks own scheduler locking, UART buffers, geometry, and
machine event application.  Neither layer parses terminal protocol bytes or
calls terminal-owned code from a machine settlement boundary.
"""
...
class RichTerminalHostHooks(Protocol):
    """Backend effects required at scheduler-owned host-port boundaries."""

    def scheduler_boundary(self) -> ContextManager[None]: ...
...
class SharedRichTerminalHost:
    """One explicit, exclusive rich-terminal attachment.

    A backend constructs this state machine but does not acquire it. A
    frontend must call :meth:`attach` with its own storage limits. Lease
    transitions serialize on the backend scheduler boundary; polling and
    bounded host admission remain independent of guest execution.
    """

    def __init__(self, hooks: RichTerminalHostHooks):
        self._hooks = hooks
    ...
    def attach(self, limits: HostPortLimits) -> TerminalHostLease:
        ...
        hooks = self._hooks
        with hooks.scheduler_boundary():
            hooks.reject_batch_reentry_locked()
            with self._lock:
            ...
            hooks.drain_completed_egress_locked()
            hooks.require_attach_ready_locked()
            ...
            with self._lock:
                ...
                hooks.bind_machine_sink_locked(self)
                return lease
        ...
        with self._lock:
            ...
            while self._scheduled:
                ...
                if isinstance(event, IngressRecord):
                    hooks.apply_uart_ingress_locked(
                        epoch,
                        event.payload,
                    )
    ...
    def _lease_close(self, token: object, epoch: int) -> AdmissionStatus:
        ...
        with hooks.scheduler_boundary():
            ...
            with self._lock:
                ...
                hooks.discard_machine_egress_locked()
                self._discard_applied_ingress_locked()
                self._retire_attachment_locked()
                self._epoch_clock = next_epoch
                hooks.bind_machine_sink_locked(None)
                return AdmissionStatus.ACCEPTED
    ...
    def _discard_applied_ingress_locked(self) -> None:
        ...
        remaining = min(
            ...
            self._hooks.pending_uart_ingress_bytes_locked(),
        )
        if remaining:
            self._hooks.discard_uart_ingress_tail_locked(remaining)
