...
def test_session_client_scopes_one_response_timeout_to_one_request():
    class FakeSocket:
        def __init__(self):
            self.timeout = 5.0
            self.timeout_writes = []
        ...
        def settimeout(self, timeout):
            ...
            self.timeout_writes.append(timeout)
    ...
    class FakeReader:
        @staticmethod
        def readline():
            return b'{"id":1,"ok":true,"result":{"state":"idle"}}\n'

    client = SessionClient("unused.sock", timeout=5.0)
    connection = FakeSocket()
    client._socket = connection
    client._reader = FakeReader()

    assert client.request_with_timeout(
        "status",
        90.0,
        detailed=False,
    ) == {"state": "idle"}
    assert connection.timeout_writes == [90.0, 5.0]
    assert connection.timeout == 5.0
    assert len(connection.payloads) == 1
    assert b'"params":{"detailed":false}' in connection.payloads[0]
    assert b"response_timeout" not in connection.payloads[0]


def test_session_client_restores_response_timeout_after_server_error():
    class FakeSocket:
        def __init__(self):
            self.timeout = 3.0
            self.timeout_writes = []
    ...
    class FakeReader:
        @staticmethod
        def readline():
            return b'{"id":1,"ok":false,"error":"not ready"}\n'

    client = SessionClient("unused.sock", timeout=3.0)
    connection = FakeSocket()
    client._socket = connection
    client._reader = FakeReader()

    with pytest.raises(RuntimeError, match="not ready"):
        client.request_with_timeout("status", 40.0)
    assert connection.timeout_writes == [40.0, 3.0]
    assert connection.timeout == 3.0


@pytest.mark.parametrize(
    "timeout",
    (True, "not-a-number", 0, -1, float("nan"), float("inf")),
)
def test_session_client_rejects_invalid_response_timeout(timeout):
    client = SessionClient("unused.sock")
    with pytest.raises((TypeError, ValueError), match="response timeout"):
        client.request_with_timeout("status", timeout)
